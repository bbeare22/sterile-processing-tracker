...
export default function MachineForm({
  ...
  useEffect(() => {
    if (initialValues) {
      reset({
        ...
        status:
          initialValues.status === 'out_of_service' ? 'inactive' : initialValues.status || 'active',
  ...
  function submit(data) {
    const payload = {
      ...data,
      lastDescaleAt: data.lastDescaleAt ? new Date(data.lastDescaleAt).toISOString() : '',
    };
    onSubmit(payload);
  }
  ...
  return (
    <form className="mf" onSubmit={handleSubmit(submit)}>
      ...
      <div className="mf__field">
        ...
        <input className="mf__input" {...register('name', { required: 'Name is required' })} />
      ...
      <div className="mf__field">
        ...
        <input type="datetime-local" className="mf__input" {...register('lastDescaleAt')} />
      </div>

      <div className="mf__actions">
        <button type="submit" disabled={submitting} className="mf__btn mf__btn--primary">
          {submitting ? 'Saving…' : 'Save'}
        </button>
        <button type="button" onClick={onCancel} className="mf__btn mf__btn--ghost">
          Cancel
        </button>
      </div>
    </form>
