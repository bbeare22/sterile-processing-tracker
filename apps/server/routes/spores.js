...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    if (status === 'pending') {
      ...
      base.$or = [{ 'spore.result': { $exists: false } }, { 'spore.result': { $in: ['', null] } }];
...
router.patch('/:id/verify', requireAuth, async (req, res) => {
  try {
    ...
    if (!cycle.spore || !cycle.spore.ran) {
      return res.status(400).json({ error: 'Spore test was not recorded for this cycle' });
    }

    cycle.spore.result = body.result;
    cycle.spore.verifiedBy = body.verifiedBy;
    cycle.spore.verifiedAt = body.verifiedAt ? new Date(body.verifiedAt) : new Date();
    await cycle.save();
