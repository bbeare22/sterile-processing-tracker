...
router.get(
  "/csv",
  requireAuth,
  // audit export is supervisor-only
  (req, res, next) => {
    ...
    try {
      ...
      if (kind === "maintenance") {
        ...
        const header = [
          ...
          // flattened helpers for washer daily/weekly when present
          "Washer Daily: R1(1-5)",
          "Washer Daily: R2(1-5)",
      ...
      if (kind === "transport") {
        ...
        const rows = await TransportTrip.find({
      ...
      if (kind === "fuel") {
        ...
        const rows = await FuelPurchase.find({
          date: { $gte: start, $lt: end },
        })
          .sort({ date: 1 })
          .lean();
      ...
      if (kind === "audit") {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="audit-${year}-${String(month).padStart(
            2,
            "0"
          )}.csv"`
        );
