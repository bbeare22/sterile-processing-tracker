...
const Maintenance = safeRequire('../models/maintenance') || safeRequire('../models/Maintenance');
...
async function buildMachineMap() {
  ...
  for (const m of machines) map[String(m._id)] = m.name || String(m._id).slice(0, 6);
  return map;
}
...
function machineName(row, machineMap) {
  ...
  if (!m) return '';
  if (typeof m === 'object' && m._id && machineMap[String(m._id)]) return machineMap[String(m._id)];
...
async function buildUserMap(data) {
  ...
  for (const u of users) {
    const name =
      (u.name || '').trim() || [u.firstName, u.lastName].filter(Boolean).join(' ').trim();
...
function displayUser(fieldValue, userMap) {
  ...
  if (typeof fieldValue === 'string' && userMap[fieldValue]) return userMap[fieldValue];
  if (typeof fieldValue === 'object' && fieldValue._id && userMap[String(fieldValue._id)]) {
...
router.get('/csv', requireAuth, async (req, res) => {
  try {
    ...
    if (
      !['spores', 'control', 'cycles', 'maintenance', 'decon', 'transport', 'fuel'].includes(kind)
    ) {
    ...
    if (kind === 'spores') {
      if (!Cycle) return res.status(501).json({ error: 'Cycles model not available.' });
    ...
    if (kind === 'control') {
      if (!ControlBI) return res.status(501).json({ error: 'ControlBI model not available.' });
      const q = { incubatedAt: { $gte: start, $lt: end } };
      if (req.query.incubatorId) q.incubatorId = String(req.query.incubatorId || '');
    ...
    if (kind === 'cycles') {
      if (!Cycle) return res.status(501).json({ error: 'Cycle model not available.' });
    ...
    if (kind === 'maintenance') {
      if (!Maintenance) return res.status(501).json({ error: 'Maintenance model not available.' });
      ...
      rows = docs.map((r) => ({
        ...
        Type: r.type || '',
        By: (r?.details?.initials || '').trim() || displayUser(r.createdBy, userMap),
    ...
    if (kind === 'decon') {
      if (!DeconLog) return res.status(501).json({ error: 'DeconLog model not available.' });
    ...
    if (kind === 'transport') {
      if (!TransportTrip)
        return res.status(501).json({ error: 'TransportTrip model not available.' });
    ...
    if (kind === 'fuel') {
      if (!FuelPurchase)
        return res.status(501).json({ error: 'FuelPurchase model not available.' });
      const q = { date: { $gte: start, $lt: end } };
      ...
      rows = docs.map((f) => ({
        ...
        Amount: f.amount ?? '',
        Vendor: f.vendor || '',
        LoggedBy: (f.signature || '').trim() || displayUser(f.createdBy, userMap),
...
router.get('/monthly', requireAuth, async (req, res) => {
  try {
    ...
    const [cycles = [], controlBIs = [], maintenance = [], deconLogs = [], trips = [], fuels = []] =
      await Promise.all([
        Cycle
    ...
    const cyclesResultCounts = countBy(cycles, (c) => (c.result || '').toLowerCase());
    const cyclesPassed =
    ...
    const fuelAmount = fuels.reduce((sum, f) => sum + (Number(f.amount) || 0), 0);
    const fuelGallons = fuels.reduce((sum, f) => sum + (Number(f.gallons) || 0), 0);
    ...
    const sectionTitle = (txt) => {
      ensureSpace(28);
      doc.fontSize(13).font('Helvetica-Bold').fillColor('#000').text(txt, margin, cursorY);
      cursorY = doc.y + 6;
    };

    const paragraph = (txt) => {
      ensureSpace(40);
      doc.fontSize(11).font('Helvetica').fillColor('#000').text(txt, margin, cursorY, {
        width: pageWidth,
      });
      cursorY = doc.y + 6;
    };
    ...
    function drawTable(columns, rows, opts = {}) {
      ...
      rows.forEach((row, rowIdx) => {
        ...
        columns.forEach((c, i) => {
          const txt = String(row[c.key] ?? '');
          doc.fillColor('#000').text(txt, xStarts[i], cursorY, { width: widths[i] });
        });
        cursorY += rowH;
      });
